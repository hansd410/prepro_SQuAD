...
def get_word_index(context, answer_start, answer_text):
	...
	if(answer_text != ' '.join(words[word_index : word_index + len(answer_text.split(' '))])):
		for i in range(fitValue):
			variable =-(fitValue/2)+i+1
			if(word_index +variable <0 or word_index +variable+len(answer_text.split(' ')) > len(context.split(' '))):
				continue
			word_index_trial = int(word_index +variable)
